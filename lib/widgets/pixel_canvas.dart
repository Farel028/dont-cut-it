...
class ArenaPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // 10. Draw Particle Effects (Smoke, Spores, Leaves, Sparks)
  ...
  void _drawLanesAndForestFloor(Canvas canvas) {
    for (int lane = 0; lane < kLaneCount; lane++) {
      ...
      final laneBgPaint = Paint()
        ..color =
            (lane % 2 == 0) ? const Color(0xFF1E3A27) : const Color(0xFF193220);
  ...
  void _drawEnemies(Canvas canvas) {
    for (final enemy in state.enemies) {
      ...
      final flashColor =
          isFlashing ? const Color(0xCCFFFFFF) : Colors.transparent;
  ...
  void _drawProjectiles(Canvas canvas) {
    for (final proj in state.projectiles) {
      ...
      } else if (proj.type == ProjectileType.damarResin) {
        final resinPaint = Paint()..color = const Color(0xFFFFB300);
        canvas.drawOval(const Rect.fromLTWH(-7, -4, 14, 8), resinPaint);
      } else if (proj.type == ProjectileType.sonokelingShard) {
        final shardPaint = Paint()..color = const Color(0xFFE53935);
        canvas.drawOval(const Rect.fromLTWH(-6, -3, 12, 6), shardPaint);
      }
